...
"""Program:  test_rabbitmqcon_start_loop.py

    Description:  Unit testing on RabbitMQCon.start_loop method in
        rabbitmq_class.py.
...
def rabbitmqcon_start_loop(cfg):

    """Function:  rabbitmqcon_start_loop

    Description:  Test start_loop method in RabbitMQCon class.
    ...
    if isinstance(RQ, rabbitmq_class.RabbitMQCon):
        ...
        if connect_status and RQ.channel.is_open:
            ...
            if tag_name.startswith("ctag"):
                print("Press CTRL+C to complete test on this unit")
                RQ.start_loop()
...
def main():
    ...
    print("\nRabbitMQCon.start_loop method unit testing...")
    rabbitmqcon_start_loop(cfg)
