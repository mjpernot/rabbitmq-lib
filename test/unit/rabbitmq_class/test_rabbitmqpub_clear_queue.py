...
"""Program:  test_rabbitmqpub_clear_queue.py

    Description:  Does unit testing on RabbitMQPub.clear_queue in
...
def rabbitmqpub_clear_queue(cfg):

    """Function:  rabbitmqpub_clear_queue

    Description:  Test clear_queue method in RabbitMQPub class.
    ...
    if isinstance(RQ, rabbitmq_class.RabbitMQPub):
        ...
        if RQ.channel.is_open and connect_status:
            ...
            if RQ.publish_msg("Test message"):
                ...
                print("\tWarning:  Failure on publishing message")
                print("\tConnection: %s" % RQ.connection)
                print("\tChannel: %s" % RQ.channel)

        else:
            print("\tFailed at is_open and connection_status")
            print("\tConnection: %s" % RQ.connection)
            print("\tChannel: %s" % RQ.channel)
...
def main():
    ...
    print("\nRabbitMQPub.clear_queue method testing...")
    rabbitmqpub_clear_queue(cfg)
